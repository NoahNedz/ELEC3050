...
while 
((RCC->CR & RCC_CR_HSIRDY) == 0);   // Wait until HSI ready
RCC->CFGR |= RCC_CFGR_SW_HSI;  
// Select HSI as system clock
    TIM10->ARR = 999; //set auto reload. 
    TIM10->PSC=1599; //set prescale. assumes 2MHz
...
int main() {
    ...
    while(1) {
        unsigned char running = READ_BIT(TIM10->CR1, TIM_CR1_CEN);//determines if counting is happening
        if (keypad.event == 7 && running) {
            CLEAR_BIT(TIM10->CR1, TIM_CR1_CEN); //toggle counting
            keypad.event = ~0;
        }
        
				else if (keypad.event == 7 && !running) {
            SET_BIT(TIM10->CR1, TIM_CR1_CEN); //toggle counting
            keypad.event = ~0;
        }				
        else if (!running && keypad.event == 5) {
            counter.first = 0;
            counter.second = 0;
            GPIOC->BSRR = 0xFF0000;
            keypad.event = ~0;
